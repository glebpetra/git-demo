    ...
    : QMainWindow(parent)
{
    ...
    connect(form, SIGNAL(ChangeSize(int, int)), SLOT(slotChangeSize(int, int)));
...
void QMyForm::slotChangeSize(int size, int pos)
{
    QString status = "Размер файла: ";
    status += QString::number(size);
    status += "  Позиция курсора: ";
    status += QString::number(pos);
